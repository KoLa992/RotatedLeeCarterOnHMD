...
log_mse = function(y, y_pred) k_mean((k_log(y_pred + k_epsilon()) - k_log(y + k_epsilon()))^2)
...
mod_save = callback_model_checkpoint(filepath = paste0("c:/r/bx_curvature_boost_logmse_", i, ".h5"), verbose = T, save_best_only = T)
...
model = load_model_hdf5(paste0("c:/r/bx_curvature_boost_logmse_", i, ".h5"))
...
log_mse = nagging_summ[Year> 1999][, mean((log(mx) - log(value))^2), keyby = .(variable, Country, Sex)]
log_mse[, min_mse := min(V1), keyby = .(Country, Sex)]
log_mse[ V1 == min_mse][, .N, keyby = variable]
