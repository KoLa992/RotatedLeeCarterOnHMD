...
CleanUp_HMD <- function(country, MinimalYear, EndOfTrainingYear, EndOfValidationYear) {
  ...
  e0data <- e0data[e0data$Year>=MinimalYear & e0data$Year <= EndOfTrainingYear,c("Year", "Total")]
...
# Function for extracting m_x estimates on training set from Rotated LC for one sex at a time
EstimateTrainingRotatedLC <- function(mx_male, mx_female, nx,
                                      e0data, p, rotation_start_e0,
                                      Gender) {
  installOrLoadPack(c("MortCast", "forecast"))
  
  li_lc <- lileecarter.estimate(mx_male, mx_female, nx = nx)
  
  rotlc <- rotate.leecarter(li_lc$bx, li_lc$ultimate.bx, e0data, p=p, e0l = rotation_start_e0)
  
  if (Gender == "male") {
    est_mx <- exp(li_lc$male$ax + rotlc %*% diag(li_lc$male$kt))
  } else {
    est_mx <- exp(li_lc$female$ax + rotlc %*% diag(li_lc$female$kt))
  }
...
ForecastLCwithGAM <- function(mx_male, mx_female, nx,
                              ...
                              IsLog,
                              Gender) {
  installOrLoadPack(c("MortCast", "forecast", "mgcv", "parallel"))
  ...
  if (Gender == "male") {
    ...
    if (length(unique(t)) < (k_base^2)) {
      ...
      smoothing_male <- bam(residual_vector ~ ti(x, k = (k_base^1), bs = basis_form) +
                            ...
                            data=residtable, family=gaussian, method="fREML", cluster=cl)
    } else{
      smoothing_male <- bam(residual_vector ~ ti(x, k = (k_base^1), bs = basis_form) +
                              ti(t, k = (k_base^1), bs = basis_form) +
...
# Function for extracting m_x estimates on training set from Rotated LC for one sex at a time
EstimateTrainingwithGAM <- function(mx_male, mx_female, nx,
                                    k_base, basis_form,
                                    IsLog, Gender) {
  installOrLoadPack(c("MortCast", "forecast", "mgcv", "parallel"))
  ...
  if (Gender == "male") {
    ...
    print("Training End Male")
    
    #plotmo::plotmo(smoothing)
    
    # GAM estimate for male residuals
    
    residtable_forecast <- data.frame(x, t)
    colnames(residtable_forecast) <- c("x", "t")
    
    predicted_residual_vector <- predict(smoothing_male, newdata = residtable_forecast)
    
    predicted_residual_matrix <- matrix(predicted_residual_vector, nrow = nrow(mx_male), ncol = ncol(mx_male))
    
    # Forecast of male m_x
    
    if (IsLog ==TRUE) {
      est_mx_male <- (li_lc$male$ax + optimbase::transpose(li_lc$male$bx) %*% as.vector(li_lc$male$kt)) -
        predicted_residual_matrix
    } else {
      est_mx_male <- exp(li_lc$male$ax + optimbase::transpose(li_lc$male$bx) %*% as.vector(li_lc$male$kt)) -
        predicted_residual_matrix
    }
    ...
    predicted_residual_matrix <- matrix(predicted_residual_vector, nrow = nrow(mx_female), ncol = ncol(mx_female))
    
    # Forecast of female m_x
    
    if (IsLog ==TRUE) {
      est_mx_female <- (li_lc$female$ax + optimbase::transpose(li_lc$female$bx) %*% as.vector(li_lc$female$kt)) -
        predicted_residual_matrix
    } else {
      est_mx_female <- exp(li_lc$female$ax + optimbase::transpose(li_lc$female$bx) %*% as.vector(li_lc$female$kt)) -
        predicted_residual_matrix
    }
