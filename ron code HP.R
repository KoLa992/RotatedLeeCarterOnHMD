...
for (i in 1:20){
  ...
  Age = layer_input(shape = 1, dtype = "int32", name = "Age")
  Age_net = Age %>%
    layer_embedding(input_dim = 101, output_dim = 8, weights = model_weights$layers[[9]] %>% get_weights(), trainable = F) %>%
    layer_flatten(name = "Age_net")
  
  Sex = layer_input(shape = 1, dtype = "int32", name = "Sex")
  Sex_net = Sex %>%
    layer_embedding(input_dim = 3, output_dim = 8, weights = model_weights$layers[[8]] %>% get_weights(), trainable = F) %>%
    layer_flatten(name = "Sex_net")
  
  Country = layer_input(shape = 1, dtype = "int32", name = "Country")
  Country_net = Country %>%
    layer_embedding(input_dim = 80, output_dim = 8, weights = model_weights$layers[[7]] %>% get_weights(), trainable = F) %>%
    layer_flatten(name = "Country_net")
  ...
  k_t = list(time_net, Country_net, Sex_net) %>% layer_concatenate() %>% 
    layer_dense(units = 64, activation = "tanh", weights = model_weights$layers[[16]] %>% get_weights(), trainable = F)%>%
    layer_batch_normalization(weights = model_weights$layers[[18]] %>% get_weights(), trainable = F) %>%
    layer_dropout(rate = 0.01)
  
  k_t = list(time_net, Country_net, Sex_net, k_t) %>%
    layer_concatenate() %>%
    layer_dense(units = 64, activation = "tanh", weights = model_weights$layers[[24]] %>% get_weights(), trainable = F)%>%
  ...
  b_x = list(Age_net, Country_net, Sex_net) %>% layer_concatenate() %>% 
    layer_dense(units = 64, activation = "tanh", weights = model_weights$layers[[17]] %>% get_weights(), trainable = F)%>%
    layer_batch_normalization(weights = model_weights$layers[[19]] %>% get_weights(), trainable = F) %>%
    layer_dropout(rate = 0.01)
  
  b_x = list(Age_net, Country_net, Sex_net, b_x)%>%
    layer_concatenate() %>%
    layer_dense(units = 64, activation = "tanh", weights = model_weights$layers[[25]] %>% get_weights(), trainable = F)%>%
  ...
  mod_save = callback_model_checkpoint(filepath = paste0("c:/r/bx_curvature_boost_hp_",  i, ".h5"), verbose = T, save_best_only = T)
  # 
  # fit <- model %>% fit(x = train_list,
  #                      y= list(output = as.matrix(train$mx_scale)),
  #                      epochs=25,
  ...
  model = load_model_hdf5(paste0("c:/r/bx_curvature_boost_hp_",  i, ".h5"))
...
non_boost = nagging[set == "test", mean((log(mx) - log(preds))^2), keyby = .(Country, Sex)] %>% 
  dcast.data.table(Country~Sex)
boost = nagging[set == "test", mean((log(mx) - log(preds_boost))^2), keyby = .(Country, Sex)] %>% 
  dcast.data.table(Country~Sex)

setnames(non_boost, names(non_boost), c("Country", "mse_male_test_NN_non_boost", 	"mse_female_test_NN_non_boost"))
setnames(boost, names(boost), c("Country", "mse_male_test_NN_boost", 	"mse_female_test_NN_boost"))

results = cbind(non_boost, boost[, 2:3])
...
comp = comp %>% merge(results)

comp %>%  fwrite("C:/R/RotatedLeeCarterOnHMD/HMD_BestResultsByCountry_Rotated+GAM+NN.csv")
...
nagging_summ_method = nagging_summ[Year >= 2000][, .(mean((log(mx) - log(value))^2)), keyby = .(Country, Sex, variable)]
nagging_summ_method[, min_V1 := min(V1), keyby = .(Country, Sex)]
nagging_summ_method[V1 == min_V1, .N, keyby = variable] 
...
nagging_summ[Year == 2015 & Country == "GBRTENW"] %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+geom_line(aes(x = Age, y = log(mx)), 
                                                                                data = nagging_summ[Year == 2015 & Country == "GBRTENW"])

stacked = nagging_summ[variable %in% c("preds_boost", "RotatedLC", "LCwithGAM"), .(value = mean(value), mx=mean(mx)), keyby = .(Country, Year, Sex, Age)]
stacked[, variable := "stacked"]

nagging_summ = rbind(nagging_summ, stacked)
